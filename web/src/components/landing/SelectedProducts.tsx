...
export default function SelectedProducts() {
  ...
  return (
    <section aria-labelledby="products-heading" className="relative px-5 py-[clamp(72px,9vw,140px)] sm:px-8">
      <div className="mx-auto max-w-6xl">
        <h2 id="products-heading" className="font-display text-display-lg font-bold text-primary">
          {copy.heading}
        </h2>
        ...
        <div className="mt-14 space-y-8">
          {copy.items.map((product, index) => (
            <Link
              key={product.name}
              href={product.href}
              className={`group grid overflow-hidden rounded-[1.75rem] border border-hairline bg-elevated/40 lg:grid-cols-2 ${
                index % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""
              }`}
            >
              <ProductVisual name={product.name} size="lg" />
              <div className="flex flex-col justify-center p-6 sm:p-10">
                <span className="w-fit rounded-full border border-accent-line px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.14em] text-accent">
                  {product.status}
                </span>
